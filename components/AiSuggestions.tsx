...
export default function AiSuggestions({
  ...
  return (
    <div className="border-t border-zinc-100 bg-gradient-to-r from-purple-50/50 via-indigo-50/40 to-blue-50/50 px-3 py-2 text-xs transition-all duration-200">
      {/* Top action bar */}
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {cooldownSeconds > 0 && (
            <span className="text-[11px] font-mono font-medium text-zinc-500 bg-white/80 px-2 py-0.5 rounded-md border border-zinc-200 shadow-2xs">
              ⏳ {cooldownSeconds}s
            </span>
          )}
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 px-3 py-1 font-medium text-white shadow-sm transition hover:from-purple-700 hover:to-indigo-700 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100"
            title={
              cooldownSeconds > 0
          ...
          {suggestions.length > 0 && (
            <button
              ...
              className="rounded-md p-1 text-zinc-400 hover:bg-zinc-200/60 hover:text-zinc-700"
      ...
      {infoMessage && (
        <div className="mt-2 flex items-center justify-between rounded-lg bg-blue-50 border border-blue-200/80 px-2.5 py-1.5 text-[11px] text-blue-700 animate-fadeIn">
          <span>💬 {infoMessage}</span>
          <button
            type="button"
            onClick={() => setInfoMessage(null)}
            className="ml-2 font-bold text-blue-400 hover:text-blue-700"
          >
            ✕
          </button>
        </div>
      ...
      {errorMessage && (
        <div className="mt-2 flex items-center justify-between rounded-lg bg-red-50 border border-red-200/80 px-2.5 py-1.5 text-[11px] text-red-600 animate-fadeIn">
      ...
      {isExpanded && suggestions.length > 0 && (
        <div className="mt-2 space-y-1.5 animate-fadeIn">
          <p className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold px-0.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5">
            {suggestions.map((item, index) => (
              <button
                ...
                disabled={disabled}
                className="group relative flex items-start gap-1.5 rounded-xl border border-purple-200/70 bg-white/90 p-2 text-left text-xs text-zinc-700 shadow-2xs transition hover:border-purple-400 hover:bg-purple-50/70 hover:shadow-xs active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-50"
                title="Click to insert into message input"
              >
                <span className="mt-0.5 text-purple-600 group-hover:scale-110 transition-transform">
                  💡
                </span>
